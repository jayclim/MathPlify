...
def get_prompt(text):
    return "You are a math assistant who is helping a student create more practice problems given a LaTeX equation. The LaTeX equation is: " + text + " The student wants to create a practice problem that is similar to the LaTeX equation that test the same ideas and concepts needed to solve the original problem.  Problems should be formatted differently. Problems should have differing methods of solving while still needing to understand the same concepts as the original problem to solve. All problems should have different numbers. List 10 practice problems that are similar to the LaTeX equation and write each new problem in LaTeX format. Do not number each problem. Seperate each problem with three backticks."